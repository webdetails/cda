...
public class DynamicTransMetaConfig
{
  ...
  private Repository connectToRepository(final RepositoryConfig rc) throws KettleException
  {
    ...
    if (!reppsitoriesMeta.readData()) throw new IllegalArgumentException("No repositories defined");

    final RepositoryMeta repositoryMeta = reppsitoriesMeta.findRepository(rc.repositoryName);
    if (repositoryMeta == null) throw new IllegalArgumentException(String.format("Repository %s not found", rc.repositoryName));
    final Repository rep = PluginRegistry.getInstance().loadClass(RepositoryPluginType.class, repositoryMeta, Repository.class);
