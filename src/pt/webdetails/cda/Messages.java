  ...
  private static ResourceBundle getBundle() {
    ...
    if (bundle == null) {
      IPentahoSession session = new StandaloneSession( "dashboards messages" ); //$NON-NLS-1$
        try {
          InputStream in = PentahoSystem.get(ISolutionRepository.class, session).getResourceInputStream("system/"+CdaContentGenerator.PLUGIN_NAME+"/messages.properties", true); //$NON-NLS-1$ //$NON-NLS-2$
