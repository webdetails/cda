 ...
 * transformation. XML snippits for Steps can easily be generated by creating
 * and configuring a step in Spoon then highlighting the step icon on the canvas
 * and copying it to the clipboard. Example use:
 * 
 * <pre>
</pre>
 * 
...
public class DynamicTransConfig
{
  ...
  /**
   * The types of config entries that can be used. Exactly one TRANS entry is
   * required.
   */
   ...
   *           if the config is not yet frozen
   */
   ...
   *          the name of the InjectorStep this RowProducer will feed
   * @param input
   *          the RowProducer that will generate rows at runtime
   * @throws IllegalAccessError
   *           if the config is frozen
  ...
  public void addInput(final String targetStepName, final RowProducerBridge input)
  {
    ...
    if (input == null) throw new IllegalArgumentException("inputRowProducer is null");
   ...
   *           if the config is frozen
   * @throws IllegalArgumentException
   *           if any input mapping is invalid
   */
   ...
   * 
   * @param targetStepName
   *          the name of the step to which this RowListener will connect
   * @param output
   *          the RowListener that will build a resultset at runtime
   * @throws IllegalAccessError
   *           if the config is frozen
   * @throws IllegalArgumentException
   *           if the output mapping is invalid
   */
  public void addOutput(final String targetStepName, final RowListener output)
  {
    if (frozen.get()) throw new IllegalAccessError("Config is frozen");
    if (isEmpty(trim(targetStepName))) throw new IllegalArgumentException("targetStepName is null");
    if (output == null) throw new IllegalArgumentException("outputListener is null");
   ...
   *           if the config is frozen
   * @throws IllegalArgumentException
   *           if any output mapping is invalid
   */
   ...
   *           if the config is frozen
   * @throws IllegalArgumentException
   *           if the config entry is invalid
   */
  ...
  /**
   * Prevents any modification to the configuration that could result in
   * inconsistent Transformation execution.
   * 
